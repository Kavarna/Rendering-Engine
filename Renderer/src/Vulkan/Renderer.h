...
private:
    std::vector<const char *> GetEnabledInstanceLayers(decltype(CreateInfo::VulkanRenderer::instanceLayers)
                                                           const &layers);
    ExtensionsOutput HandleEnabledInstanceExtensions(decltype(CreateInfo::VulkanRenderer::instanceExtensions)
                                                         const &extensions);

    std::vector<const char *> GetEnabledDeviceLayers(decltype(CreateInfo::VulkanRenderer::instanceLayers)
                                                         const &layers);
    ExtensionsOutput HandleEnabledDeviceExtensions(decltype(CreateInfo::VulkanRenderer::instanceExtensions)
                                                       const &extensions);
...
private:
    ...
    std::vector<VkImageLayout>
        mSwapchainImageLayouts; // Used in command buffer to figure out what to do with transitions
