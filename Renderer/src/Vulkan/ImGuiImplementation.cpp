...
struct ImGui_ImplVulkan_ViewportData
{
    ...
    {
        WindowOwned = false; memset(&RenderBuffers, 0, sizeof(RenderBuffers));
    }
    ~ImGui_ImplVulkan_ViewportData()
    { }
};
...
static void CreatePipeline(VkFormat pipelineFormat, Vulkan::RootSignature* rootSignature, std::unique_ptr<Vulkan::Pipeline>& pipeline)
{
...
static void SetupRenderState(ImDrawData* drawData, Vulkan::CommandList* cmdList, PerFrameBuffers* rb, int fbWidth, int fbHeight)
{
    auto* bd = GetBackendData();
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* drawData, Vulkan::CommandList* cmdList)
{
    ...
    if (drawData->TotalVtxCount > 0)
    {
        ...
        if (rb->VertexBuffer == nullptr || rb->VertexBuffer->GetSize() < vertexSize)
        {
            ...
            rb->VertexBuffer = std::make_unique<Vulkan::Buffer>(
                sizeof(ImDrawVert), drawData->TotalVtxCount, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
        ...
        if (rb->IndexBuffer == nullptr || rb->IndexBuffer->GetSize() < indexSize)
        {
            ...
            rb->IndexBuffer = std::make_unique<Vulkan::Buffer>(
                sizeof(ImDrawIdx), drawData->TotalIdxCount, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT
                );
        }
...
bool ImGui_ImplVulkan_CreateFontsTexture(Vulkan::CommandList* cmdList)
{
    ...
    {
        info2D.width = width; info2D.height = height;
    ...
    bd->UploadBuffer = std::make_unique<Vulkan::Buffer>(4, width * height,
    ...
    ImTextureID fontID = ImGui_ImplVulkan_AddTexture(renderer->GetFontSampler(), bd->FontImage->GetImageView(VK_IMAGE_ASPECT_COLOR_BIT));
